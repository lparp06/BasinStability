...
Purpose
-------
Run the original GenerateDynamics.py (written by Dr. Jeremie Fish) functions and save baseline outputs
for comparison against future implementations.

Author: Leah Parparov
"""
...
def record_solution_info(sol, t, expected_state_variables):
    ...
    add_to_summary(
        "State dimension check", "PASS" if state_dimension_pass else "FAIL", indent=2
    )
...
def record_sync_consistency(final_distance, sync_times):
    ...
    for tol, sync_time in sync_times.items():
        ...
        add_to_summary(
            f"Tol={tol} final_distance < tol", final_distance_below_tol, indent=2
        )
...
def coupling_sweep():
    ...
    coupling_values = [
        ...
        1.25,
        1.5,
    ]  # anything over 1 causes an error!
